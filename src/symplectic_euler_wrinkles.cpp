...
namespace argus
{
	void SymplecticEulerWrinkles::initWrinkleMesh(std::vector<std::unique_ptr<DynamicsObject>>& objects, bool amps, bool dphis)
	{
	...
	void SymplecticEulerWrinkles::optimize(std::vector<std::unique_ptr<DynamicsObject>>& objects, bool amps, bool dphis, bool positions)
	{
	...
	void SymplecticEulerWrinkles::getCollisionFreeState(std::vector<std::unique_ptr<Obstacle>>& obstacles, VectorN& X, VectorN& V, const VectorN& Xold, const VectorN& W, const std::vector<trimesh::triangle_t>& triangles, Real timestep, std::vector<int>* collVerts, unsigned int frame, unsigned int substep)
	{
		/**
	...
	void SymplecticEulerWrinkles::resolveCollisions(std::vector<std::unique_ptr<Obstacle>>& obs, VectorN& updated_pos, const VectorN& oldPositions, VectorN& velocities, const VectorN& W, const std::vector<trimesh::triangle_t>& triangles, Real timestep, nlohmann::json& state, unsigned int frame, std::vector<unsigned int>& collVerts, unsigned int substep)
	{
		/**
	...
	void SymplecticEulerWrinkles::init(std::vector<std::unique_ptr<DynamicsObject>>& dynamicObjects, std::vector<std::unique_ptr<argus::Obstacle>>& obstacles, nlohmann::json& params)
	{
		m_kdamp_amps = 0;
		m_kdamp_pos = 0;
		m_substeps = 1;
	...
	void SymplecticEulerWrinkles::step(Real timeStep, std::vector<std::unique_ptr<DynamicsObject>>& dynamicObjects, std::vector<std::unique_ptr<argus::Obstacle>>& obs, nlohmann::json& state, unsigned int frame)
	{
		// m_stats["optimizeDphis"] = 0;

		auto ttotal1 = argus::util::captureCurrentTime();
		Real substep_kdamp = 1.0 - std::pow(1.0 - m_kdamp_pos, 1.0 / m_substeps);
	...
	void SymplecticEulerWrinkles::updatePositions(int objid, Real timeStep,
	    ...
	    nlohmann::json& state, unsigned int frame, int step)
	{
		...
		if (nobs > 0)
		{
			...
			// project vertex amplitudes
			// for (auto&& vt : collverts)
			// {
			// 	object.getAmpVec()(vt) = 0;
			// 	object.getAmpVelocities()(vt) = -object.getAmpVelocities()(vt) / timeStep;
			// }
		}
